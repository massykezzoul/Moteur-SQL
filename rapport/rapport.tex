...
Avant toute chose, il faut commencer par lire les données. Pour cela il faut mettre en \eo uvre une stratégie afin de stocker efficacement les données pour pouvoir y accéder et d’y effectuer les opérations demandées par l'utilisateur, le plus aisément possible.
...
Une fois le code compiler, un éxecutable sera créé (\textit{sql}). Vous pourrez ainsi utiliser notre application de deux façon différentes :
...
En éxecutant l'application de cette façon, elle charge les tables, données en argument, en mémoire puis vous laisse la main pour donner la requête SQL. Vous aurez le choix entre quelques commandes. Si votre commande commence par : \texttt{select} alors l'application considére que lui avez donné la requête SQL et l'éxecute sur les données en mémoire. Les commandes \texttt{csv} et \texttt{txt} affiche, respectivement, le fichier \textit{resultat.csv} et \textit{resultat.txt} sur la sortie standard. Pour plus d'information, la commande \texttt{help} affiche la liste des commandes disponible. La commande \texttt{exit} arrête l'application.
...
Une optimisation en mémoire de notre application semble l'une des premières perspectives sur les quelles nous pouvons nous pencher. En effet, notre application ne permet pas d'éxecuter des jointure entre des tables assez conséquentes. Afin d'optimiser celà, plusieurs solution sont possible.

\begin{itemize}
	\item Commencer par l'éxecution d'une projection partielle. C'est à dire supprimer les attributs (colonnes) qui n'apparaîssent pas sur les conditions de sélection. Celà réduira, grandement dans certains cas, la taille des tables sur laquelle la jointure sera executées.
	\item Ensuite dans la même optique de réduction de la taille des tables, éxecuter les conditions de sélection (hors conditions de jointure) sur chaque tables individuellment.
	\item Enfin, après toutes ces manipulation, il est envisageable d'écrire le resultat du produit cartésien dans un fichier temporaire, et pas directement en mémoire, afin de ne jamais surcharger la mémoire vive. Mais celà entrainera, en contre partie, un temps d'éxecution plus longs.
\end{itemize}

\subsubsection{Opérateur}

Il est aussi possible d'ajouté certains opérateurs à notre selection telles que :
\begin{description}
	...
	\item[IN] L’opérateur logique qui vérifie si une colonne est égale à une des valeurs comprise dans set de valeurs déterminés.
\end{description}
...
Nous avons aussi pensé à rajoter des fonctions d'agrégation, mais le temps ne nous l'a pas premis.
